import{Ht as e,Ln as t,Sn as n,_n as r,bn as i,bt as a,gn as o,in as s,kt as c,vt as l,wn as u}from"./vendor-three-DBnxpZKO.js";var d=[1e3,1e4,5e4,1e5],f=60,p=300,m=600,h=220;function g(){return new c(4,4,4)}function _(e,t){let n=t=>{let n=Math.sin((e+1)*12.9898+t*78.233)*43758.5453;return n-Math.floor(n)};return[(n(1)*2-1)*m,(n(2)*2-1)*m,(n(3)*2-1)*m]}function v(e,t,n,i){let a=[];for(let o=0;o<i;o++){let s=new r(t,n),[c,l,u]=_(o,i);s.position.set(c,l,u),s.updateMatrix(),s.matrixAutoUpdate=!1,e.add(s),a.push(s)}return{dispose:()=>a.forEach(t=>e.remove(t))}}function y(e,t,r,i){let a=new s(t,r,i);a.frustumCulled=!1;let o=new n;for(let e=0;e<i;e++){let[t,n,r]=_(e,i);o.position.set(t,n,r),o.updateMatrix(),a.setMatrixAt(e,o.matrix)}return a.instanceMatrix.needsUpdate=!0,e.add(a),{dispose:()=>e.remove(a)}}var b=`
...
`)}async function E(){let n=document.createElement(`div`);n.style.cssText=`position:fixed;inset:0;background:#0a0d12;color:#cde;font:13px/1.5 monospace;z-index:99999;padding:16px;overflow:auto;white-space:pre-wrap`,n.textContent=`Phase 0 cull benchmark — initializing WebGPU…
`,document.body.appendChild(n);let r=e=>{n.textContent+=e+`
`,console.log(e)},s=document.createElement(`canvas`);s.width=1280,s.height=720;let c=new l({canvas:s,antialias:!1,trackTimestamp:!0});await c.init(),c.setSize(1280,720,!1),c._benchGpuMs=NaN;let f=!1,p=()=>{!c.backend?.trackTimestamp||f||(f=!0,c.resolveTimestampsAsync?.(`render`)?.then(e=>{Number.isFinite(e)&&e>=0&&(c._benchGpuMs=e)}).catch(()=>{}).finally(()=>{f=!1}))},m=new t,_=new u(60,1280/720,.5,4e3);m.add(new a(16777215,.4));let b=new e(16777215,1);b.position.set(1,1,1),m.add(b);let x=g(),C=new i({color:13402197,roughness:.8}),E=0,D=new o;function O(){E+=.01,_.position.set(Math.cos(E)*h,40,Math.sin(E)*h),_.lookAt(0,0,0),_.updateMatrixWorld()}function k(){return _.updateMatrixWorld(),D.multiplyMatrices(_.projectionMatrix,_.matrixWorldInverse),new Float32Array(D.elements)}let A=()=>c.info.render.drawCalls,j=[],M={renderer:c,drawCallsOf:A};for(let e of d){r(`\n=== N = ${e} ===`);{let t=v(m,x,C,e),n=await w(`baseline`,e,()=>{O(),c.render(m,_),p()},M);t.dispose(),j.push(n),r(`baseline   cpu=${n.cpuMs} gpu=${n.gpuMs} draws=${n.drawCalls}`)}{let t=y(m,x,C,e),n=await w(`instanced`,e,()=>{O(),c.render(m,_),p()},M);t.dispose(),j.push(n),r(`instanced  cpu=${n.cpuMs} gpu=${n.gpuMs} draws=${n.drawCalls}`)}{let t;try{t=await S(c,x,e,k);let n=await w(`gpucull`,e,()=>{O(),t.frame(),p()},M);t.dispose(),n.drawCalls=1,j.push(n),r(`gpucull    cpu=${n.cpuMs} gpu=${n.gpuMs} draws=1 (indirect)`)}catch(n){r(`gpucull    FAILED: ${n.message}`),j.push({label:`gpucull`,n:e,cpuMs:NaN,gpuMs:NaN,drawCalls:1,heapPerFrameKB:NaN}),t?.dispose?.()}}}let N=T(j);r(`

===== PHASE 0 RESULTS =====
`+N);let P={};j.forEach(e=>{P[`${e.label}:${e.n}`]=e});let F=[];for(let e of d){if(e<1e4)continue;let t=P[`baseline:${e}`],n=P[`instanced:${e}`],r=P[`gpucull:${e}`],i=e=>(e&&Number.isFinite(e.cpuMs)?e.cpuMs:0)+(e&&Number.isFinite(e.gpuMs)?e.gpuMs:0),a=i(t),o=i(r),s=i(n);o>0&&a/o>=2?F.push(`N=${e}: gpucull ${(a/o).toFixed(2)}× faster than baseline → PASS`):s>0&&o>0&&Math.abs(o-s)/s<.15?F.push(`N=${e}: gpucull≈instanced → win is instancing only`):F.push(`N=${e}: gpucull ${(a/Math.max(o,.001)).toFixed(2)}× vs baseline → below 2× gate`)}return r(`
===== GATE =====
`+F.join(`
`)),r(`
