...
import engineSoundGeneratorWorkletUrl from './engine_sound_generator_worklet_webassembly.js?url';
import sineWaveSoundGeneratorWorkletUrl from './sine_wave_sound_generator_worklet_webassembly.js?url';

const SPEED_OF_SOUND = 343; // speed of sound in m/s

function resolveWorkletModuleUrl(fileName, basePath = '') {
   if (!basePath) {
      return new URL(fileName, import.meta.url).href;
   }

   const baseUrl = new URL(basePath.endsWith('/') ? basePath : `${basePath}/`, import.meta.url);
...
//      this.setFilter(this.delayNode);
   ...
   static _load(loadingManager, listener, path) {
      ...
      return listener.context.audioWorklet.addModule(path)
         .catch((error) => {
            loadingManager.itemError(path);
            throw error;
         })
         .finally(() => {
            loadingManager.itemEnd(path);
         });
   }
...
class SineWaveSoundGenerator extends SoundGenerator {
   ...
   static load (loadingManager, listener, basePath="") {
      const moduleUrl = basePath
         ? resolveWorkletModuleUrl('sine_wave_sound_generator_worklet_webassembly.js', basePath)
         : sineWaveSoundGeneratorWorkletUrl;
      return SoundGenerator._load(loadingManager, listener, moduleUrl);
   }
...
class EngineSoundGenerator extends SoundGenerator {
   ...
   */
   ...
   static load (loadingManager, listener, basePath="") {
      const moduleUrl = basePath
         ? resolveWorkletModuleUrl('engine_sound_generator_worklet_webassembly.js', basePath)
         : engineSoundGeneratorWorkletUrl;
      return SoundGenerator._load(loadingManager, listener, moduleUrl);
   }
