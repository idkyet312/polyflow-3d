import{Rt as e,Xt as t,Zt as n,en as r,ft as i,gn as a,in as o,nn as s,rt as c,tt as l,wt as u}from"./vendor-three-Cin1Ncu_.js";var d=[1e3,1e4,5e4,1e5],f=60,p=300,m=600,h=220;function g(){return new i(4,4,4)}function _(e,t){let n=t=>{let n=Math.sin((e+1)*12.9898+t*78.233)*43758.5453;return n-Math.floor(n)};return[(n(1)*2-1)*m,(n(2)*2-1)*m,(n(3)*2-1)*m]}function v(e,t,r,i){let a=[];for(let o=0;o<i;o++){let s=new n(t,r),[c,l,u]=_(o,i);s.position.set(c,l,u),s.updateMatrix(),s.matrixAutoUpdate=!1,e.add(s),a.push(s)}return{dispose:()=>a.forEach(t=>e.remove(t))}}function y(t,n,r,i){let a=new e(n,r,i);a.frustumCulled=!1;let o=new s;for(let e=0;e<i;e++){let[t,n,r]=_(e,i);o.position.set(t,n,r),o.updateMatrix(),a.setMatrixAt(e,o.matrix)}return a.instanceMatrix.needsUpdate=!0,t.add(a),{dispose:()=>t.remove(a)}}var b=`
...
`)}async function E(){let e=document.createElement(`div`);e.style.cssText=`position:fixed;inset:0;background:#0a0d12;color:#cde;font:13px/1.5 monospace;z-index:99999;padding:16px;overflow:auto;white-space:pre-wrap`,e.textContent=`Phase 0 cull benchmark — initializing WebGPU…
`,document.body.appendChild(e);let n=t=>{e.textContent+=t+`
`,console.log(t)},i=document.createElement(`canvas`);i.width=1280,i.height=720;let s=new l({canvas:i,antialias:!1,trackTimestamp:!0});await s.init(),s.setSize(1280,720,!1),s._benchGpuMs=NaN;let f=!1,p=()=>{!s.backend?.trackTimestamp||f||(f=!0,s.resolveTimestampsAsync?.(`render`)?.then(e=>{Number.isFinite(e)&&e>=0&&(s._benchGpuMs=e)}).catch(()=>{}).finally(()=>{f=!1}))},m=new a,_=new o(60,1280/720,.5,4e3);m.add(new c(16777215,.4));let b=new u(16777215,1);b.position.set(1,1,1),m.add(b);let x=g(),C=new r({color:13402197,roughness:.8}),E=0,D=new t;function O(){E+=.01,_.position.set(Math.cos(E)*h,40,Math.sin(E)*h),_.lookAt(0,0,0),_.updateMatrixWorld()}function k(){return _.updateMatrixWorld(),D.multiplyMatrices(_.projectionMatrix,_.matrixWorldInverse),new Float32Array(D.elements)}let A=()=>s.info.render.drawCalls,j=[],M={renderer:s,drawCallsOf:A};for(let e of d){n(`\n=== N = ${e} ===`);{let t=v(m,x,C,e),r=await w(`baseline`,e,()=>{O(),s.render(m,_),p()},M);t.dispose(),j.push(r),n(`baseline   cpu=${r.cpuMs} gpu=${r.gpuMs} draws=${r.drawCalls}`)}{let t=y(m,x,C,e),r=await w(`instanced`,e,()=>{O(),s.render(m,_),p()},M);t.dispose(),j.push(r),n(`instanced  cpu=${r.cpuMs} gpu=${r.gpuMs} draws=${r.drawCalls}`)}{let t;try{t=await S(s,x,e,k);let r=await w(`gpucull`,e,()=>{O(),t.frame(),p()},M);t.dispose(),r.drawCalls=1,j.push(r),n(`gpucull    cpu=${r.cpuMs} gpu=${r.gpuMs} draws=1 (indirect)`)}catch(r){n(`gpucull    FAILED: ${r.message}`),j.push({label:`gpucull`,n:e,cpuMs:NaN,gpuMs:NaN,drawCalls:1,heapPerFrameKB:NaN}),t?.dispose?.()}}}let N=T(j);n(`

===== PHASE 0 RESULTS =====
`+N);let P={};j.forEach(e=>{P[`${e.label}:${e.n}`]=e});let F=[];for(let e of d){if(e<1e4)continue;let t=P[`baseline:${e}`],n=P[`instanced:${e}`],r=P[`gpucull:${e}`],i=e=>(e&&Number.isFinite(e.cpuMs)?e.cpuMs:0)+(e&&Number.isFinite(e.gpuMs)?e.gpuMs:0),a=i(t),o=i(r),s=i(n);o>0&&a/o>=2?F.push(`N=${e}: gpucull ${(a/o).toFixed(2)}× faster than baseline → PASS`):s>0&&o>0&&Math.abs(o-s)/s<.15?F.push(`N=${e}: gpucull≈instanced → win is instancing only`):F.push(`N=${e}: gpucull ${(a/Math.max(o,.001)).toFixed(2)}× vs baseline → below 2× gate`)}return n(`
===== GATE =====
`+F.join(`
`)),n(`
