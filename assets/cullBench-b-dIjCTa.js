import{Qt as e,Zt as t,_n as n,an as r,ft as i,rn as a,rt as o,tn as s,tt as c,wt as l,zt as u}from"./vendor-three-o_HWTQwa.js";var d=[1e3,1e4,5e4,1e5],f=60,p=300,m=600,h=220;function g(){return new i(4,4,4)}function _(e,t){let n=t=>{let n=Math.sin((e+1)*12.9898+t*78.233)*43758.5453;return n-Math.floor(n)};return[(n(1)*2-1)*m,(n(2)*2-1)*m,(n(3)*2-1)*m]}function v(t,n,r,i){let a=[];for(let o=0;o<i;o++){let s=new e(n,r),[c,l,u]=_(o,i);s.position.set(c,l,u),s.updateMatrix(),s.matrixAutoUpdate=!1,t.add(s),a.push(s)}return{dispose:()=>a.forEach(e=>t.remove(e))}}function y(e,t,n,r){let i=new u(t,n,r);i.frustumCulled=!1;let o=new a;for(let e=0;e<r;e++){let[t,n,a]=_(e,r);o.position.set(t,n,a),o.updateMatrix(),i.setMatrixAt(e,o.matrix)}return i.instanceMatrix.needsUpdate=!0,e.add(i),{dispose:()=>e.remove(i)}}var b=`
...
`,document.body.appendChild(e);let i=t=>{e.textContent+=t+`
`,console.log(t)},a=document.createElement(`canvas`);a.width=1280,a.height=720;let u=new c({canvas:a,antialias:!1,trackTimestamp:!0});await u.init(),u.setSize(1280,720,!1),u._benchGpuMs=NaN;let f=!1,p=()=>{!u.backend?.trackTimestamp||f||(f=!0,u.resolveTimestampsAsync?.(`render`)?.then(e=>{Number.isFinite(e)&&e>=0&&(u._benchGpuMs=e)}).catch(()=>{}).finally(()=>{f=!1}))},m=new n,_=new r(60,1280/720,.5,4e3);m.add(new o(16777215,.4));let b=new l(16777215,1);b.position.set(1,1,1),m.add(b);let x=g(),C=new s({color:13402197,roughness:.8}),E=0,D=new t;function O(){E+=.01,_.position.set(Math.cos(E)*h,40,Math.sin(E)*h),_.lookAt(0,0,0),_.updateMatrixWorld()}function k(){return _.updateMatrixWorld(),D.multiplyMatrices(_.projectionMatrix,_.matrixWorldInverse),new Float32Array(D.elements)}let A=()=>u.info.render.drawCalls,j=[],M={renderer:u,drawCallsOf:A};for(let e of d){i(`\n=== N = ${e} ===`);{let t=v(m,x,C,e),n=await w(`baseline`,e,()=>{O(),u.render(m,_),p()},M);t.dispose(),j.push(n),i(`baseline   cpu=${n.cpuMs} gpu=${n.gpuMs} draws=${n.drawCalls}`)}{let t=y(m,x,C,e),n=await w(`instanced`,e,()=>{O(),u.render(m,_),p()},M);t.dispose(),j.push(n),i(`instanced  cpu=${n.cpuMs} gpu=${n.gpuMs} draws=${n.drawCalls}`)}{let t;try{t=await S(u,x,e,k);let n=await w(`gpucull`,e,()=>{O(),t.frame(),p()},M);t.dispose(),n.drawCalls=1,j.push(n),i(`gpucull    cpu=${n.cpuMs} gpu=${n.gpuMs} draws=1 (indirect)`)}catch(n){i(`gpucull    FAILED: ${n.message}`),j.push({label:`gpucull`,n:e,cpuMs:NaN,gpuMs:NaN,drawCalls:1,heapPerFrameKB:NaN}),t?.dispose?.()}}}let N=T(j);i(`

===== PHASE 0 RESULTS =====
`+N);let P={};j.forEach(e=>{P[`${e.label}:${e.n}`]=e});let F=[];for(let e of d){if(e<1e4)continue;let t=P[`baseline:${e}`],n=P[`instanced:${e}`],r=P[`gpucull:${e}`],i=e=>(e&&Number.isFinite(e.cpuMs)?e.cpuMs:0)+(e&&Number.isFinite(e.gpuMs)?e.gpuMs:0),a=i(t),o=i(r),s=i(n);o>0&&a/o>=2?F.push(`N=${e}: gpucull ${(a/o).toFixed(2)}× faster than baseline → PASS`):s>0&&o>0&&Math.abs(o-s)/s<.15?F.push(`N=${e}: gpucull≈instanced → win is instancing only`):F.push(`N=${e}: gpucull ${(a/Math.max(o,.001)).toFixed(2)}× vs baseline → below 2× gate`)}return i(`
===== GATE =====
`+F.join(`
`)),i(`
