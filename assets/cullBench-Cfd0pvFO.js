import{C as e,Dt as t,Et as n,Ht as r,Mt as i,Rt as a,V as o,d as s,dn as c,it as l,p as u}from"./three.webgpu-kNNYs9Fv.js";var d=[1e3,1e4,5e4,1e5],f=60,p=300,m=600,h=220;function g(){return new e(4,4,4)}function _(e,t){let n=t=>{let n=Math.sin((e+1)*12.9898+t*78.233)*43758.5453;return n-Math.floor(n)};return[(n(1)*2-1)*m,(n(2)*2-1)*m,(n(3)*2-1)*m]}function v(e,n,r,i){let a=[];for(let o=0;o<i;o++){let s=new t(n,r),[c,l,u]=_(o,i);s.position.set(c,l,u),s.updateMatrix(),s.matrixAutoUpdate=!1,e.add(s),a.push(s)}return{dispose:()=>a.forEach(t=>e.remove(t))}}function y(e,t,n,r){let i=new l(t,n,r);i.frustumCulled=!1;let o=new a;for(let e=0;e<r;e++){let[t,n,a]=_(e,r);o.position.set(t,n,a),o.updateMatrix(),i.setMatrixAt(e,o.matrix)}return i.instanceMatrix.needsUpdate=!0,e.add(i),{dispose:()=>e.remove(i)}}var b=`
...
`,console.log(t)},a=document.createElement(`canvas`);a.width=1280,a.height=720;let l=new s({canvas:a,antialias:!1,trackTimestamp:!0});await l.init(),l.setSize(1280,720,!1),l._benchGpuMs=NaN;let f=!1,p=()=>{!l.backend?.trackTimestamp||f||(f=!0,l.resolveTimestampsAsync?.(`render`)?.then(e=>{Number.isFinite(e)&&e>=0&&(l._benchGpuMs=e)}).catch(()=>{}).finally(()=>{f=!1}))},m=new c,_=new r(60,1280/720,.5,4e3);m.add(new u(16777215,.4));let b=new o(16777215,1);b.position.set(1,1,1),m.add(b);let x=g(),C=new i({color:13402197,roughness:.8}),E=0,D=new n;function O(){E+=.01,_.position.set(Math.cos(E)*h,40,Math.sin(E)*h),_.lookAt(0,0,0),_.updateMatrixWorld()}function k(){return _.updateMatrixWorld(),D.multiplyMatrices(_.projectionMatrix,_.matrixWorldInverse),new Float32Array(D.elements)}let A=()=>l.info.render.drawCalls,j=[],M={renderer:l,drawCallsOf:A};for(let e of d){t(`\n=== N = ${e} ===`);{let n=v(m,x,C,e),r=await w(`baseline`,e,()=>{O(),l.render(m,_),p()},M);n.dispose(),j.push(r),t(`baseline   cpu=${r.cpuMs} gpu=${r.gpuMs} draws=${r.drawCalls}`)}{let n=y(m,x,C,e),r=await w(`instanced`,e,()=>{O(),l.render(m,_),p()},M);n.dispose(),j.push(r),t(`instanced  cpu=${r.cpuMs} gpu=${r.gpuMs} draws=${r.drawCalls}`)}{let n;try{n=await S(l,x,e,k);let r=await w(`gpucull`,e,()=>{O(),n.frame(),p()},M);n.dispose(),r.drawCalls=1,j.push(r),t(`gpucull    cpu=${r.cpuMs} gpu=${r.gpuMs} draws=1 (indirect)`)}catch(r){t(`gpucull    FAILED: ${r.message}`),j.push({label:`gpucull`,n:e,cpuMs:NaN,gpuMs:NaN,drawCalls:1,heapPerFrameKB:NaN}),n?.dispose?.()}}}let N=T(j);t(`

===== PHASE 0 RESULTS =====
`+N);let P={};j.forEach(e=>{P[`${e.label}:${e.n}`]=e});let F=[];for(let e of d){if(e<1e4)continue;let t=P[`baseline:${e}`],n=P[`instanced:${e}`],r=P[`gpucull:${e}`],i=e=>(e&&Number.isFinite(e.cpuMs)?e.cpuMs:0)+(e&&Number.isFinite(e.gpuMs)?e.gpuMs:0),a=i(t),o=i(r),s=i(n);o>0&&a/o>=2?F.push(`N=${e}: gpucull ${(a/o).toFixed(2)}× faster than baseline → PASS`):s>0&&o>0&&Math.abs(o-s)/s<.15?F.push(`N=${e}: gpucull≈instanced → win is instancing only`):F.push(`N=${e}: gpucull ${(a/Math.max(o,.001)).toFixed(2)}× vs baseline → below 2× gate`)}return t(`
===== GATE =====
`+F.join(`
`)),t(`
