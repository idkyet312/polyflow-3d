import{$t as e,Et as t,Vt as n,an as r,at as i,bn as a,en as o,mt as s,rn as c,rt as l,sn as u}from"./vendor-three-Dsx19tqm.js";var d=[1e3,1e4,5e4,1e5],f=60,p=300,m=600,h=220;function g(){return new s(4,4,4)}function _(e,t){let n=t=>{let n=Math.sin((e+1)*12.9898+t*78.233)*43758.5453;return n-Math.floor(n)};return[(n(1)*2-1)*m,(n(2)*2-1)*m,(n(3)*2-1)*m]}function v(e,t,n,r){let i=[];for(let a=0;a<r;a++){let s=new o(t,n),[c,l,u]=_(a,r);s.position.set(c,l,u),s.updateMatrix(),s.matrixAutoUpdate=!1,e.add(s),i.push(s)}return{dispose:()=>i.forEach(t=>e.remove(t))}}function y(e,t,i,a){let o=new n(t,i,a);o.frustumCulled=!1;let s=new r;for(let e=0;e<a;e++){let[t,n,r]=_(e,a);s.position.set(t,n,r),s.updateMatrix(),o.setMatrixAt(e,s.matrix)}return o.instanceMatrix.needsUpdate=!0,e.add(o),{dispose:()=>e.remove(o)}}var b=`
...
`,console.log(e)},o=document.createElement(`canvas`);o.width=1280,o.height=720;let s=new l({canvas:o,antialias:!1,trackTimestamp:!0});await s.init(),s.setSize(1280,720,!1),s._benchGpuMs=NaN;let f=!1,p=()=>{!s.backend?.trackTimestamp||f||(f=!0,s.resolveTimestampsAsync?.(`render`)?.then(e=>{Number.isFinite(e)&&e>=0&&(s._benchGpuMs=e)}).catch(()=>{}).finally(()=>{f=!1}))},m=new a,_=new u(60,1280/720,.5,4e3);m.add(new i(16777215,.4));let b=new t(16777215,1);b.position.set(1,1,1),m.add(b);let x=g(),C=new c({color:13402197,roughness:.8}),E=0,D=new e;function O(){E+=.01,_.position.set(Math.cos(E)*h,40,Math.sin(E)*h),_.lookAt(0,0,0),_.updateMatrixWorld()}function k(){return _.updateMatrixWorld(),D.multiplyMatrices(_.projectionMatrix,_.matrixWorldInverse),new Float32Array(D.elements)}let A=()=>s.info.render.drawCalls,j=[],M={renderer:s,drawCallsOf:A};for(let e of d){r(`\n=== N = ${e} ===`);{let t=v(m,x,C,e),n=await w(`baseline`,e,()=>{O(),s.render(m,_),p()},M);t.dispose(),j.push(n),r(`baseline   cpu=${n.cpuMs} gpu=${n.gpuMs} draws=${n.drawCalls}`)}{let t=y(m,x,C,e),n=await w(`instanced`,e,()=>{O(),s.render(m,_),p()},M);t.dispose(),j.push(n),r(`instanced  cpu=${n.cpuMs} gpu=${n.gpuMs} draws=${n.drawCalls}`)}{let t;try{t=await S(s,x,e,k);let n=await w(`gpucull`,e,()=>{O(),t.frame(),p()},M);t.dispose(),n.drawCalls=1,j.push(n),r(`gpucull    cpu=${n.cpuMs} gpu=${n.gpuMs} draws=1 (indirect)`)}catch(n){r(`gpucull    FAILED: ${n.message}`),j.push({label:`gpucull`,n:e,cpuMs:NaN,gpuMs:NaN,drawCalls:1,heapPerFrameKB:NaN}),t?.dispose?.()}}}let N=T(j);r(`
